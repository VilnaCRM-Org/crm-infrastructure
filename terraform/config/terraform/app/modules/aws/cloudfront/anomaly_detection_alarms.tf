resource "aws_cloudwatch_metric_alarm" "cloudfront_requests_anomaly_detection" {
  ...
  alarm_description   = "This metric monitors Anomaly Requests in CloudFront for CRM"
  alarm_actions       = [aws_sns_topic.crm_cloudwatch_alarm_notifications.arn]
  ...
  metric_query {
    ...
    metric {
      ...
      dimensions = {
        DistributionId = aws_cloudfront_distribution.crm_this.id
        Region         = "Global"
      }
    }
  }
...
resource "aws_cloudwatch_metric_alarm" "wafv2_allowed_requests_anomaly_detection" {
  ...
  evaluation_periods  = 5
  threshold_metric_id = "e1"
  alarm_description   = "This metric monitors WAFV2 Allowed Requests for CRM"
  alarm_actions       = [aws_sns_topic.crm_cloudwatch_alarm_notifications.arn]
...
resource "aws_cloudwatch_metric_alarm" "wafv2_blocked_requests_anomaly_detection" {
  ...
  evaluation_periods  = 5
  threshold_metric_id = "e1"
  alarm_description   = "This metric monitors WAFV2 Blocked Requests for CRM"
  alarm_actions       = [aws_sns_topic.crm_cloudwatch_alarm_notifications.arn]
