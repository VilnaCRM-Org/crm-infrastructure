data "aws_iam_policy_document" "sandbox_cleanup_function_policy" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:logs:${data.aws_region.current.id}:${local.account_id}:log-group:/aws/lambda/sandbox-crm-cleanup-lambda:*"
    ]
  }
  ...
  statement {
    ...
    resources = [
      "arn:aws:lambda:${data.aws_region.current.id}:${local.account_id}:function:sandbox-crm-cleanup-lambda"
    ]
  }
