resource "aws_lambda_function" "sandbox_cleanup_lambda" {
  ...
  #checkov:skip=CKV_AWS_116: DLQ needs additional topics, too complex for sandbox-crm cleanup lambda function
  #checkov:skip=CKV_AWS_272: Code-signing is not needed for sandbox-crm cleanup lambda function
  ...
  function_name                  = "sandbox-crm-cleanup-lambda"
...
resource "aws_iam_role" "sandbox_cleanup_function_role" {
  name = "sandbox-crm-cleanup-function-role"
...
resource "aws_iam_policy" "sandbox_cleanup_function_policy" {
  name        = "sandbox-crm-cleanup-function-policy"
