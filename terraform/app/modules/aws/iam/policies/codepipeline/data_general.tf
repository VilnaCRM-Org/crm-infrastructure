...
data "aws_iam_policy_document" "general_policy_doc" {
  ...
  statement {
    ...
    resources = ["arn:aws:s3:::terraform-state-${local.account_id}-${var.region}-${var.environment}/main/${var.region}/${var.environment}/stacks/ci-cd-infrastructure-crm/terraform.tfstate"]
  }
  ...
  statement {
    ...
    resources = [
      ...
      "arn:aws:cloudwatch:${var.region}:${local.account_id}:alarm:${var.ci_cd_crm_project_name}-lambda-reports-errors",
      "arn:aws:cloudwatch:${var.region}:${local.account_id}:alarm:${var.ci_cd_crm_project_name}-lambda-reports-invocations-anomaly-detection",
      "arn:aws:cloudwatch:${var.region}:${local.account_id}:alarm:${var.ci_cd_crm_project_name}-lambda-reports-throttles-anomaly-detection",
      "arn:aws:cloudwatch:${var.region}:${local.account_id}:alarm:${var.ci_cd_crm_project_name}-lambda-reports-duration-anomaly-detection",
    ]
  }
  statement {
  ...
  statement {
    ...
    resources = [
      "arn:aws:events:${var.region}:${local.account_id}:rule/sandbox-crm-cleanup-rule"
    ]
  }
} 