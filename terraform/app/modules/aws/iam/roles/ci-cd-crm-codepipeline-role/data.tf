...
data "aws_iam_policy_document" "codepipeline_policy_document" {
  statement {
    ...
    resources = [
      "${var.s3_bucket_arn}/*",
      "${var.s3_bucket_arn}",
      "arn:aws:s3:::${var.crm_bucket_name}",
      "arn:aws:s3:::${var.crm_bucket_name}/*",
      "arn:aws:s3:::staging.${var.crm_bucket_name}",
      "arn:aws:s3:::staging.${var.crm_bucket_name}/*",
  ...
  statement {
    ...
    condition {
      ...
      values = [
        "${var.source_repo_owner}/${var.source_repo_name}",
        "${var.source_repo_owner}/${var.crm_content_repo_name}",
      ]
    }
  }
  ...
  statement {
    ...
    resources = ["arn:aws:lambda:${data.aws_region.current.id}:${local.account_id}:function:ci-cd-crm-${var.environment}-reports-notification"]
  }
  statement {
    ...
    resources = ["arn:aws:cloudwatch:${var.region}:${local.account_id}:alarm:crm-${var.region}-s3-objects-anomaly-detection"]
  }
  statement {
