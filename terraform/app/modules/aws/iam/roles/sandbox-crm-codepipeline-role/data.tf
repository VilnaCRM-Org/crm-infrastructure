...
data "aws_iam_policy_document" "terraform_role_document" {
  statement {
    ...
    principals {
      type        = "AWS"
      identifiers = ["arn:${data.aws_partition.current.partition}:iam::${local.account_id}:role/sandbox-crm-${var.environment}${var.codepipeline_role_name_suffix}"]
    }
  }
}
...
data "aws_iam_policy_document" "codepipeline_policy_document" {
  ...
  statement {
    ...
    resources = ["arn:${data.aws_partition.current.partition}:codepipeline:${var.region}:${local.account_id}:sandbox-crm-creation"]
  }
  ...
  statement {
    ...
    resources = ["arn:aws:lambda:${data.aws_region.current.id}:${local.account_id}:function:sandbox-crm-cleanup-lambda"]
  }
}