module "ci_cd_crm_s3_artifacts_bucket" {
  ...
  codepipeline_role_arn = module.ci_cd_crm_codepipeline_iam_role.role_arn

  tags = var.tags
}

module "ci_cd_crm_codepipeline_iam_role" {
  source = "../../modules/aws/iam/roles/ci-cd-crm-codepipeline-role"
  ...
  s3_bucket_arn                  = module.ci_cd_crm_s3_artifacts_bucket.arn
...
module "ci_cd_crm_codebuild" {
  source = "../../modules/aws/codebuild/stages"

  project_name   = var.ci_cd_crm_project_name
  build_projects = local.ci_cd_crm_build_projects

  region      = var.region
  environment = var.environment

  s3_bucket_name = module.ci_cd_crm_s3_artifacts_bucket.bucket
  role_arn       = module.ci_cd_crm_codepipeline_iam_role.role_arn
...
module "ci_cd_crm_codepipeline" {
  ...
  stages = var.ci_cd_crm_stage_input
  ...
  depends_on = [module.ci_cd_crm_codebuild, module.ci_cd_crm_s3_artifacts_bucket]
}
