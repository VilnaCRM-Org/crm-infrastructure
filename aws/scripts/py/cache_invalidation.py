#!/usr/bin/env python3
"""
CloudFront Cache Invalidation Script for CRM

Performs blue-green cache invalidation for CloudFront CRM app distributions.
...
class CloudFrontCacheInvalidator:
    ...
    def _classify_distribution(
        ...
        if self._is_staging_distribution(distribution, origin_domains):
            self.logger.info("Found CRM staging distribution: %s", dist_id)
            return Environment.STAGING

        self.logger.info("Found CRM production distribution: %s", dist_id)
    ...
    def _log_found_distributions(
        ...
        self.logger.info("Using production distribution only: %s", production_id)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="CloudFront blue-green cache invalidation for CRM"
    )
    parser.add_argument(
        "--log-level",
