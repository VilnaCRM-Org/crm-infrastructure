...
aws events put-targets \
  ...
  --targets "[{
      \"Id\": \"$unique_id\",
      \"Arn\": \"arn:aws:lambda:$region:$account_id:function:sandbox-crm-cleanup-lambda\",
      \"InputTransformer\": {
        \"InputPathsMap\": {},
...
existing_permission=$(aws lambda get-policy --function-name sandbox-crm-cleanup-lambda 2>/dev/null | grep "$rule_name" || true)
